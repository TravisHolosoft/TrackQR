...
public sealed class Helpers
{
    ...
    #region CreateSpaceVectors
    public static List<Vector3> CreateSpaceVectors(float xLen, float yLen, float zLen, float spacing)
    {
        List<Vector3> spaceVectors = new List<Vector3>();
        for (float zD = 0; zD < zLen; zD += spacing)
        {
            for (float yD = 0; yD < yLen; yD += spacing)
            {
                for (float xD = 0; xD < xLen; xD += spacing)
                {
                    spaceVectors.Add(new Vector3(xD, yD, zD));
                }
            }
        }
        return spaceVectors;
